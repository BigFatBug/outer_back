...
function refush(){
	...
	success: function(resultData){
		...
		$.each(resultData.data, function(i, item){
			new_html += '<tr><td>'+item.id+'</td><td>'+item.name+'</td><td>'+item.group_id+'</td><td>'+new Date(item.ctime).toLocaleString()+'</td><td><a><button class="btn" onclick="remove('+item.id+')">删除</button></a></td></tr>';
		})
